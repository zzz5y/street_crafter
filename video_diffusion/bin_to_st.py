...
def convert(ckpt_path="path_to/pytorch_model.bin", save_path="ckpts/vista.safetensors"):
    vista_bin = torch.load(ckpt_path, map_location="cpu")  # only contains model weights
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ckpt_path",
        type=str,
        default="path_to/pytorch_model.bin",
        help="path to the pytorch model"
    )
    parser.add_argument(
        "--save_path",
        type=str,
        default="ckpts/vista.safetensors",
        help="path to save the SafeTensors model"
    )
    args = parser.parse_args()
    convert(args.ckpt_path, args.save_path)
