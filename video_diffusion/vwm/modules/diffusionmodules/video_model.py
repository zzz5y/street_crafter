...
from vwm.util import default, repeat_as_img_seq, append_dims
...
class VideoUNet(nn.Module):
    def __init__(
        ...
        self.condition_input_blocks = nn.ModuleList(
            [
                TimestepEmbedSequential(conv_nd(dims, in_channels // 2, model_channels, 3, padding=1)),
                TimestepEmbedSequential(conv_nd(dims, model_channels, model_channels, 3, padding=1)),
            ]
        )
        zero_module(self.condition_input_blocks[1][0])
    ...
    def forward(
        ...
        for i, module in enumerate(self.input_blocks):
            ...
            if i == 0 and guidance is not None:
                guidance_h = guidance['input']
                for guidance_module in self.condition_input_blocks:
                    guidance_h = guidance_module(
                        guidance_h,
                        emb,
                        context=context,
                        time_context=time_context,
                ...
                guidance_scale = append_dims(guidance['scale'], guidance_h.ndim)
                h = h + guidance_h * guidance_scale
