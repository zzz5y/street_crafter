...
class PandasetDataset(BaseDataset):
    def __init__(
            self, 
            data_root="/lpai/volumes/jointmodel/yanyunzhi/data/pandaset", 
            anno_file=None, 
            postfix=None,
        ...
        print("Pandaset loaded:", len(self))
    ...
    def build_data_dict(self, image_seq, guide_seq, guide_mask_seq, sample_dict):
        ...
        data_dict = {
            ...
            "guide_mask_seq": torch.stack(guide_mask_seq),
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.num_frames):
            current_index = i
            guide_mask_path = self.get_guidance_mask_path(sample_dict, current_index)
            guide_mask = self.preprocess_image(guide_mask_path, self.guide_preprocessor)
            guide_mask_seq.append(guide_mask[0:1])
            
        data_dict = self.build_data_dict(image_seq, guide_seq, guide_mask_seq, sample_dict)
