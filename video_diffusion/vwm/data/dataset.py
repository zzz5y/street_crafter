...
from .subsets import YouTubeDataset, NuScenesDataset, WaymoDataset, NuScenesDataset_Overfit, PandasetDataset
try:
...
def dataset_mapping(data_root: str, subset_list: list, target_height: int, target_width: int, num_frames: int, split: str):
    datasets = list()
    for subset_name in subset_list:
        ...
        elif subset_name == "Waymo":
            datasets.append(
                WaymoDataset(target_height=target_height, target_width=target_width, num_frames=num_frames, split=split)
            )
        elif subset_name == "Pandaset":
            datasets.append(
                PandasetDataset(target_height=target_height, target_width=target_width, num_frames=num_frames, split=split)
            )
        else:
...
class MultiSourceSamplerDataset(Dataset):
    def __init__(self, data_root, subsets, probs, samples_per_epoch=1000, target_height=320, target_width=576, num_frames=25, split="train"):
        self.subsets = dataset_mapping(data_root, subsets, target_height, target_width, num_frames, split)
...
class Sampler(LightningDataModule):
    def __init__(self, batch_size, data_root=None, num_workers=0, prefetch_factor=2, shuffle=True, subsets=None, probs=None,
                 samples_per_epoch=None, target_height=320, target_width=576, num_frames=25, postfix=None):
        super().__init__()
        ...
        self.train_dataset = MultiSourceSamplerDataset(
            data_root=data_root, subsets=subsets, probs=probs, samples_per_epoch=samples_per_epoch, 
            target_height=target_height, target_width=target_width, num_frames=num_frames, split="train"
        )
        
        self.val_dataset = MultiSourceSamplerDataset(
            data_root=data_root, subsets=subsets, probs=probs, samples_per_epoch=samples_per_epoch,
            target_height=target_height, target_width=target_width, num_frames=num_frames, split="val"
        )
        
        self.test_dataset = MultiSourceSamplerDataset(
            data_root=data_root, subsets=subsets, probs=probs, samples_per_epoch=samples_per_epoch,
            target_height=target_height, target_width=target_width, num_frames=num_frames, split="test"
        )
