...
class DiffusionEngine(LightningModule):
    def __init__(
            ...
            slow_spatial_layers_scale: float = 0.1,
            slow_temporal_layers: bool = False,
            slow_temporal_layers_scale: float = 0.1,
        ...
        self.slow_spatial_layers_scale = slow_spatial_layers_scale
        self.slow_temporal_layers = slow_temporal_layers
        self.slow_temporal_layers_scale = slow_temporal_layers_scale
    ...
    def get_guidance(self, batch, scale=1.0, ucg_rate=0.0):
        gudiance_shape = batch[self.guidance_key].shape
        if len(gudiance_shape) != 4:  # is an image sequence
            b, t = batch[self.guidance_key].shape[:2]
            assert gudiance_shape[1] == self.num_frames
            batch[self.guidance_key] = rearrange(batch[self.guidance_key], "b t c h w -> (b t) c h w")
        else:
            t = self.num_frames
            b = batch[self.guidance_key].shape[0] // t
        
        guidance = self.encode_first_stage(batch[self.guidance_key])
        guidance_dict = dict()
        guidance_dict['input'] = guidance
        
        scales = torch.ones((b, t)).to(batch[self.guidance_key].device) * scale
        for i in range(b):
            scales[i] = scales[i] if random.random() > ucg_rate else torch.zeros_like(scales[i])
        guidance_dict['scale'] = rearrange(scales, "b t -> (b t)")
    ...
    def shared_step(self, batch: Dict) -> Any:
        ...
        # conditions
        if self.guidance_key in batch:
            batch['guidance'] = self.get_guidance(batch, ucg_rate=0.15)
        ...
        if self.slow_spatial_layers:
            param_dicts = [
                ...
                },
                {
                    ...
                    "lr": lr * self.slow_spatial_layers_scale
                }
            ]
            ...
            param_dicts = [
                {
                    "params": [p for n, p in self.model.named_parameters() if "time_stack" in n],
                    "lr": lr * self.slow_temporal_layers_scale
                },
                {
                    "params": [p for n, p in self.model.named_parameters() if "time_stack" not in n]
                }
            ...
            param_dicts = [
                ...
                {
                    "params": [p for n, p in self.model.named_parameters() if "cond_time_stack_embed" in n]
                }
            ]
        else:
            param_dicts = [
    ...
    def log_images(
        ...
        if self.guidance_key in batch:
            guidance_c = self.get_guidance(batch, scale=1.0)
            guidance_c['input'] = guidance_c['input'][:N].to(self.device)
            guidance_c['scale'] = guidance_c['scale'][:N].to(self.device)
            c['guidance'] = guidance_c
            guidance_uc = self.get_guidance(batch, scale=0.0)
            guidance_uc['input'] = guidance_uc['input'][:N].to(self.device)
            guidance_uc['scale'] = guidance_uc['scale'][:N].to(self.device)
            uc['guidance'] = guidance_uc
            
            # log["guidance"] = log["guidance_mp4"] = batch[self.guidance_key][:N] * 2. - 1.
            log["guidance"] = log["guidance_mp4"] = batch[self.guidance_key][:N]
