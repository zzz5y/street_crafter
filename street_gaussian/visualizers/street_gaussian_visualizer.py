...
from street_gaussian.utils.img_utils import visualize_depth_numpy
...
class StreetGaussianisualizer():
    ...
    def visualize_novel_view(self, result, camera: Camera):
        ...
        if self.save_image:
            ...
            torchvision.utils.save_image(acc.float(), os.path.join(self.result_dir, f'{id:06d}_acc.png'))
    ...
    def visualize_diff(self, result, camera: Camera):
        ...
        mask = camera.guidance['mask'] if 'mask' in camera.guidance else torch.ones_like(rgb[0:1]).bool()
