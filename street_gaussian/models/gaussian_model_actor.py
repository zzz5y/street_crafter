...
from street_gaussian.utils.general_utils import quaternion_to_matrix, inverse_sigmoid, matrix_to_quaternion, get_expon_lr_func, quaternion_raw_multiply
from street_gaussian.utils.sh_utils import RGB2SH, IDFT
...
class GaussianModelActor(GaussianModel):
    def __init__(
        ...
        self.obj_id = obj_meta['object_id']
        ...
        xyz = torch.tensor(self.bbox, device='cuda', dtype=torch.float)
        self.min_xyz, self.max_xyz = -xyz / 2., xyz / 2.

        extent = max(length * 1.5 / cfg.data.box_scale, width * 1.5 / cfg.data.box_scale, height) / 2.
        self.extent = torch.tensor([extent], device='cuda', dtype=torch.float)
    ...
    def get_features_fourier(self, frame=0):
        ...
        idft_base = IDFT(time, self.fourier_dim)[0].to('cuda', non_blocking=True)
    ...
    def create_from_pcd(self, pcd, spatial_lr_scale: float):
        ...
        fused_point_cloud = torch.tensor(np.asarray(pointcloud_xyz), device='cuda', dtype=torch.float)
        fused_color = RGB2SH(torch.tensor(np.asarray(pointcloud_rgb), device='cuda', dtype=torch.float))

        # features = torch.zeros((fused_color.shape[0], 3,
        #                         (self.max_sh_degree + 1) ** 2 * self.fourier_dim), device='cuda', dtype=torch.float)
        # features[:, :3, 0] = fused_color
        features_dc = torch.zeros((fused_color.shape[0], 3, self.fourier_dim), device='cuda', dtype=torch.float)
        features_rest = torch.zeros(fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2 - 1, device='cuda', dtype=torch.float)
        ...
        opacities = inverse_sigmoid(0.1 * torch.ones((fused_point_cloud.shape[0], 1), device='cuda', dtype=torch.float))
        semantics = torch.zeros((fused_point_cloud.shape[0], self.num_classes), device='cuda', dtype=torch.float)
