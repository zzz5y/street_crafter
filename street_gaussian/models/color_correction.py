...
from street_gaussian.utils.general_utils import get_expon_lr_func, matrix_to_axis_angle
# fmt: off
...
class ColorCorrection(nn.Module):
    def __init__(self, metadata):
        super().__init__()
        self.identity_matrix = torch.eye(4, device='cuda', dtype=torch.float)[:3]  # [3, 4]
        ...
        if self.config.use_mlp:
            ...
            self.affine_trans = nn.Parameter(torch.eye(4, device='cuda', dtype=torch.float)[:3].unsqueeze(0).repeat(num_corrections, 1, 1)).requires_grad_(True)
            self.affine_trans_sky = nn.Parameter(torch.eye(4, device='cuda', dtype=torch.float)[:3].unsqueeze(0).repeat(num_corrections, 1, 1)).requires_grad_(True)
